...
class Corpus(object):
    ...
    def __init__(self):
        ...
        self.vectorizer = corpus[0]
        self.train_docs = corpus[1]
        self.test_docs = corpus[2]
    ...
    def document_vector(self, doc_id):
        ...
        if doc_id in self.train_docs.keys():
            return self.train_docs[doc_id]
        elif doc_id in self.test_docs.keys():
            return self.test_docs[doc_id]
        else:
    ...
    def train_matrix(self, category=None, include_ids=False):
        ...
        Args:
            category (optional): The category to get the training matrix for.
                If not provided, returns a complete map from category names to training matrices.
        ...
        """
        return self._get_matrix(self.train_docs, category, include_ids)

    def test_matrix(self, category=None, include_ids=False):
        """Returns the matrix of vectorized testing documents.

        See docstring for train_matrix.
        """
        return self._get_matrix(self.test_docs, category, include_ids)

    def _get_matrix(self, docs, category, include_ids):
    ...
    def _as_category_matrix_dict(docs):
        ...
        """
        docs_by_category = {category:([], []) for category in all_categories()}
